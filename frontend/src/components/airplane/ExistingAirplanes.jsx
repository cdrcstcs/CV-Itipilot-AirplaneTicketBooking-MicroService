...
const ExistingAirplanes = () => {
  ...
  useEffect(() => {
    if (selectedAirplaneType === "") {
      ...
      const filteredAirplanes = airplanes.filter(
        (airplane) => airplane.airplaneType === selectedAirplaneType
      );
  ...
  const handleDelete = async (airplaneId) => {
    try {
      ...
      if (result === "") {
        setSuccessMessage(`Airplane No ${airplaneId} was deleted`);
        fetchAirplanes();
      } else {
  ...
  const calculateTotalPages = () => {
    const totalAirplanes =
      filteredAirplanes.length > 0 ? filteredAirplanes.length : airplanes.length;
          ...
          <section className="mt-5 mb-5 container">
            ...
            <Row>
              <Col md={6} className="mb-2 md-mb-0">
                <AirplaneFilter
                  data={airplanes}
                  setFilteredData={setFilteredAirplanes}
                />
              </Col>
              ...
              <tbody>
                {currentAirplanes.map((airplane) => (
                  <tr key={airplane.id} className="text-center">
                    ...
                    <td>{new Date(airplane.departureDate).toLocaleDateString()}</td>
                    <td>{new Date(airplane.landingDate).toLocaleDateString()}</td>
                    <td className="gap-2">
